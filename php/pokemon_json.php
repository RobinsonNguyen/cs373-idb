<?php
	...
	$create = mysqli_prepare($con, 'CREATE TABLE ALL_POKEMON(POKEMON_ID INT NOT NULL, POKEMON_NAME VARCHAR(50) NOT NULL, POKEMON_HP INT NOT NULL, POKEMON_ATK INT NOT NULL, POKEMON_DEF INT NOT NULL, POKEMON_SPATK INT NOT NULL, POKEMON_SPDEF INT NOT NULL, POKEMON_SPD INT NOT NULL, POKEMON_HEIGHT INT, POKEMON_WEIGHT INT, POKEMON_IMG VARCHAR(256) NOT NULL, PRIMARY KEY(POKEMON_ID));') or die(mysqli_error($con));

    mysqli_stmt_execute($drop) or die(mysqli_error($con));
    mysqli_stmt_execute($create) or die(mysqli_error($con));

    $st = mysqli_prepare($con, 'INSERT INTO ALL_POKEMON(POKEMON_ID, POKEMON_NAME, POKEMON_HP, POKEMON_ATK, POKEMON_DEF, POKEMON_SPATK, POKEMON_SPDEF, POKEMON_SPD, POKEMON_HEIGHT, POKEMON_WEIGHT, POKEMON_IMG) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)') or die(mysqli_error($con));

    mysqli_stmt_bind_param($st, 'isiiiiiiiis', $id, $name, $hp, $attack, $defense, $spatk, $spdef, $spd, $height, $weight, $img);
